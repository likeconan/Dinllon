...
export function openInviteDialog() {
    return {type: 'OPEN_INVITE_ACTIVITY'}
}
...
export function applyActivity(activity) {
    return function (dispatch) {
        dinaxios({url: 'activities/join', method: 'POST', data: activity}).then((data) => {
            dispatch(closeDialog());
            dispatch(showToast({className: 'success-toast', message: 'apply_activity_success'}));
        });
    }
}
